/**
 * Routing Tool Integration - Deno Compatible
 * 
...
export const routingTool = tool(
  async ({ jobs, vehicle, options = {} }) => {
    try {
      ...
      const ROUTING_ENGINE_URL = Deno.env.get('VROOM_API_URL') || 'http://localhost:3000/vroom';
      ...
      try {
        // Try to make actual HTTP request to VROOM engine
        const response = await fetch(ROUTING_ENGINE_URL, {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
          ...
          console.log('✅ Routing Tool: Real VROOM response received');
          return vroomResult;
        } else {
          console.warn('⚠️ VROOM service unavailable, using mock response');
          throw new Error('VROOM service unavailable');
        }
      } catch (error) {
        console.warn('⚠️ VROOM connection failed, using mock response:', error);
        // Fall back to mock response if VROOM is unavailable
...
/**
 * Simulate VROOM API response for fallback when VROOM is unavailable
 */
...
function generateMockGeometry(jobs: any[], vehicle: any): string {
  // In production, this will be actual encoded polyline from OSRM
