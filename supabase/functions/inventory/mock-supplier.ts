...
/**
 * Main supplier query function - exported for use as a tool
 * @param supplier - The supplier to query (e.g., 'home_depot', 'lowes')
 * @param items - Array of items to check
 * @param location - Optional location for store distance calculation
 * @returns Promise<SupplierAPIResponse>
 */ export async function querySupplier(supplier, items, location) {
  try {
    console.log(`🏪 Mock Supplier API: Checking ${supplier} for ${items.length} items`);
    // Simulate API processing delay
    await new Promise((resolve)=>setTimeout(resolve, 200));
    const supplierData = MOCK_SUPPLIERS[supplier];
    if (!supplierData) {
    ...
    const stores = supplierData.stores.map((store)=>({
        ...store,
        distance_miles: location ? calculateDistance(location.latitude, location.longitude, store.coordinates.latitude, store.coordinates.longitude) * 0.621371 : 5.2
      }));
    // Generate item availability
    const itemAvailability = items.map((item)=>{
      const itemKey = findItemKey(item.name);
      const mockItem = MOCK_INVENTORY[itemKey];
      if (!mockItem) {
...
/**
 * Simplified supplier query function for single items
 * @param supplier - The supplier to query
 * @param itemName - Single item name to check
 * @param quantity - Optional quantity (defaults to 1)
 * @returns Promise<SupplierAPIResponse>
 */ export async function querySupplierForItem(supplier, itemName, quantity = 1) {
  return querySupplier(supplier, [
    {
      name: itemName,
      quantity
    }
 ...
 * @returns Array of available supplier names
 */ export function getAvailableSuppliers() {
  return Object.keys(MOCK_SUPPLIERS);
}
/**
 * Get supplier information
 * @param supplier - The supplier name
 * @returns Supplier information or null if not found
 */ export function getSupplierInfo(supplier) {
  return MOCK_SUPPLIERS[supplier] || null;
}
/**
 * Legacy wrapper for backward compatibility
 * @deprecated Use querySupplier instead
 */ export const mockSupplierAPI = {
  async invoke (input) {
    return querySupplier(input.supplier, input.items, input.location);
  }
};
/**
 ...
 */ function generateGenericItem(itemName, supplier) {
  const genericBrands = [
    'Generic',
    'ProTech',
    'BuildMaster'
 ...
 */ function generateSKU(supplier, itemKey) {
  const prefixes = {
    'home_depot': 'HD',
    'lowes': 'LW',
    'grainger': 'GR',
